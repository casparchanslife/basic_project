...
using System.Linq;
using System.Text;
using Learn.Lib.Helpers.Grid.Enums;
using Learn.Lib.Helpers.Grid.Extensions;

namespace Learn.Lib.Helpers.Grid
{
    public class Column
    {
        ...
        /// </summary>
        /// <param name = "searchTerms">Searchterm to add to dropdownlist</param>
        /// <param name = "defaultSearchValue">DefaultSearchValue</param>
        /// <param name = "isFirtsNull">first is null</param>
        public Column SetSearchTerms(string[] searchTerms,bool isFirtsNull)
        {
            _searchTerms = searchTerms.ToDictionary(searchterm => searchterm);
            _SelectFirtsNull = isFirtsNull;
            return this;
        }
        ...
        public override string ToString()
        {
            ...
            if (_searchType.HasValue)
            {
                if (_searchType == Searchtype.Select)
                {
                    if (_searchTerms != null)
                    {
                        ...
                        {
                            searchOptions.Add("value", "\"" + string.Format(string.Join(";", _searchTerms.Select(s => s.Key + ":" + s.Value).ToArray())) + "\"");
                        }
                    }
                    else
                    {
            ...
            if (_searchType.HasValue)
            {
                if (_searchType == Searchtype.Datepicker)
                {
                    ...
                    {
                        //searchOptions.Add("dataInit", "function(el){$(el).datepicker({changeYear: true, onSelect: function() {var sgrid = $('###gridid##')[0]; sgrid.triggerToolbar();}, format: 'dd-mm-yy'});}");
                        //searchOptions.Add("dataInit", "function(el){$(el).datepicker({format: 'dd-mm-yy'});}");
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({pickTime: false, format: 'DD-MM-YYYY'});}");
                    }
                    else
                    {
                        //searchOptions.Add("dataInit", "function(el){$(el).datepicker({changeYear: true, onSelect: function() {var sgrid = $('###gridid##')[0]; sgrid.triggerToolbar();}, format: '" + _searchDateFormat + "'});}");
                        //searchOptions.Add("dataInit", "function(el){$(el).datepicker({format: '" + _searchDateFormat + "'});}");
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({pickTime: false, format: '" + _searchDateFormat + "'});}");
                    }
                }

                if (_searchType == Searchtype.Datetimepicker)
                {
                    if (_searchDateFormat.IsNullOrWhiteSpace())
                    {
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({format: 'DD-MM-YYYY hh:mm'});}");
                    }
                    else
                    {
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({format: '" + _searchDateFormat + "'});}");
                    }
                }

                if (_searchType == Searchtype.Timepicker)
                {
                    if (_searchDateFormat.IsNullOrWhiteSpace())
                    {
                        //searchOptions.Add("dataInit", "function(el){jQuery(el).datetimepicker({pickDate: false, format: 'dd-mm-yy'});}");
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({pickDate: false, format: 'hh:mm'});}");
                    }
                    else
                    {
                        //searchOptions.Add("dataInit", "function(el){jQuery(el).datetimepicker({pickDate: false, format: '" + _searchDateFormat + "'});}");
                        searchOptions.Add("dataInit", "function(el){$(el).datetimepicker({pickDate: false, format: '" + _searchDateFormat + "'});}");
                    }
                }
            }
            ...
            // Search Option: sopt // Phoebe 2015-10-14
            if (_searchType.HasValue)
            {
                if (_searchType == Searchtype.Select)
                {
                ...
                else if (_searchType == Searchtype.Datetimepicker || _searchType == Searchtype.Datepicker || _searchType == Searchtype.Timepicker)
                {
                    //_searchOptions.AddRange(new List<string>() { "eq", "ne", "lt", "le", "gt", "ge" });
                    _searchOptions.AddRange(new List<string>() { "eq","lt", "le", "gt", "ge" });
                }
                else if (_searchType == Searchtype.Number)
                {
                    _searchOptions.AddRange(new List<string>() { "eq", "ne", "lt", "le", "gt", "ge" });
                }
                else
